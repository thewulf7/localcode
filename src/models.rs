...
pub fn scan_ollama_cache() -> Vec<DiscoveredModel> {
    ...
    let manifests_dir = ollama_dir.join("models").join("manifests").join("registry.ollama.ai");
                                        ...
                                        if blob_path.exists() {
                                            if let Ok(metadata) = fs::metadata(&blob_path) {
                                                ...
                                                if let Some(parent) = path.parent() {
                                                    if let Some(repo_name) = parent.parent().and_then(|p| p.file_name()).and_then(|f| f.to_str()) {
                                                        if let Some(tag_name) = parent.file_name().and_then(|f| f.to_str()) {
                                                            let final_tag = path.file_name().unwrap_or_default().to_str().unwrap_or("latest");
