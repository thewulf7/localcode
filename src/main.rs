...
pub enum Commands {
    /// Perform initial setup and model selection
    Setup(SetupArgs),
...
pub struct SetupArgs {
    ...
    /// Specify the model to use directly (e.g. llama3-8b-instruct)
    #[arg(short, long)]
    ...
    match args.command {
        ...
        Commands::Start => {
            ...
            if config.run_in_docker {
                println!("{} {} with llama.cpp in Docker on port {}...", 
                    style("🐳 Starting").blue(), 
                    style(&config.model_name).magenta().bold(), 
                    style(config.port).yellow()
                );
                let (repo, file) = runner::extract_hf_repo_and_file(&config.model_name, &config.quant).await;
                
                if !config.models_dir.exists() {
                    tokio::fs::create_dir_all(&config.models_dir).await.unwrap_or(());
                }

                if let Err(e) = runner::start_llama_cpp_docker(&repo, &config.models_dir, file.as_deref(), config.port).await {
        ...
        Commands::Setup(setup_args) => {
            println!("\n{}\n", style("✨ Welcome to OpenCode Local LLM Setup! ✨").cyan().bold());
            ...
            let provider_url = format!("http://localhost:{}", user_config.port);
            config::configure_opencode(&user_config.model_name, &provider_url).await?;

            // 5. Download default skills
            config::download_initial_skills(&user_config.selected_skills).await?;

            // 6. Save configuration to disk
            config::save_localcode_config(&user_config).await?;
